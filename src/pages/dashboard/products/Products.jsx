...
const Products = ({ title = "التصنيفات" }) => {
  ...
  const getAllData = async () => {
    // setIsLoading(true);
    try {
      let dataA = await baseUrl.get("api/Product", {
        headers: {
          Authorization: `Bearer ${window.localStorage.getItem("token")}`,
        },
      });
      setIsLoading(false);
      setDataT(dataA.data.data);
    } catch (error) {
    } finally {
      setIsLoading(false);
    }
    ...
    <>
      ...
      <Card className="p-2">
        <div className="md:flex justify-between items-center mb-6">
          ...
          <div>
            <AddProductModal getAllData={getAllData} />
